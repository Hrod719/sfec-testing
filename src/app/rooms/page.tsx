  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 no-print">
        <div>
          <h2 className="text-xl font-semibold text-white mb-1">Testing Room Rosters</h2>
          <p className="text-sm text-slate-400">One sheet per room. Print button generates all rooms.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {TESTING_ROOMS.map(room => {
          ...
          return (
            <div key={room} className="rounded-xl overflow-hidden border border-slate-700 print-break">
              {/* Room header */}
              ...
              {/* Roster table */}
              <div className="overflow-x-auto" style={{ background: '#0F1923' }}>
                    ...
                    {roomStudents.map((s, i) => {
                      ...
                      return (
                        <tr key={s.id} className={noExit ? 'alert-row' : ''}>
                          <td className="text-slate-500 text-xs text-center">{i + 1}</td>
                          <td>
                            <span className="font-semibold text-white">{getStudentFullName(s)}</span>
                            {noExit && <span className="ml-2 text-red-400 text-xs font-bold">⚠</span>}
                          </td>
                          <td className="text-center text-slate-300">{s.grade}</td>
                          <td className="text-center text-slate-300">{s.period}</td>
                          <td className="text-xs text-slate-400">{s.teacher_name.split(' ')[0]}</td>
